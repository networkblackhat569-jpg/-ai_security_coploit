...
import socket
import database  # NEW: SQLite persistence layer (database.py)
                        ...
                        if not captured_raw:
                            ...
                            {{
                              ...
                              "security_analysis": {{
                                ...
                                "x_frame_options": {{ "status": "missing", "risk_note": "Factual explanation details here" }}
                              }},
                              "risk_scores": {{ "csp_score": 30, "hsts_score": 0, "xfo_score": 15, "total_risk_score": 45 }}
                            }}
                            ...
                            """
                            if llm:
                                with st.spinner("🤖 Executing header audit analytics..."):
                                    header_report = llm.invoke(header_analysis_schema_prompt)
                                    try:
                                        cleaned_json = re.search(r"(\{.*\})", header_report, re.DOTALL)
                                        if cleaned_json:
                                            report_data = json.loads(cleaned_json.group(1))
                                            scores = report_data.get("risk_scores", {})
                                            csp_s = scores.get("csp_score", 0)
                                            hsts_s = scores.get("hsts_score", 0)
                                            xfo_s = scores.get("xfo_score", 0)
                                            
                                            compliance_figure = go.Figure(data=[
                                                go.Bar(x=["CSP", "HSTS", "X-Frame-Options"], y=[csp_s, hsts_s, xfo_s], marker_color=['#ef4444', '#f59e0b', '#10b981'], width=0.4)
                                            ])
                                            compliance_figure.update_layout(title=f"Target: {target_domain} Security Risk Breakdown", template="plotly_dark", height=280, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', margin=dict(l=10, r=10, t=45, b=10))
                                            st.session_state.latest_chart = compliance_figure
                                    except:
                                        pass
                                    ...
                                    add_history(target_domain, "HTTP Header Audit", "LOW", captured_raw)
                            else:
                                st.error("AI Language model core link offline.")
                    else:
                        st.error("Please provide a valid domain target (e.g. headers target.com)")

                # ROUTE 2: PORT PROFILE COMMAND ROUTER (NMAP DIRECT LINK)
                elif "nmap" in prompt_lower or "scan" in prompt_lower:
                    dom_match = re.search(r"([a-zA-Z0-9.-]+\.[a-zA-Z]{2,})", prompt)
                    if dom_match:
                        target_domain = dom_match.group(1)
                        add_log(f"Nmap perimeter footprint profiling initiated for: {target_domain}")
                        with st.spinner("🔒 Running perimeter Nmap scan..."):
                            try:
                                resolved_ip = socket.gethostbyname(target_domain)
                                nmap_out = run_nmap(resolved_ip)
                            except:
                                nmap_out = run_nmap(target_domain)
                        st.session_state.total_scans += 1
                        
                        if llm:
                            with st.spinner("🤖 Analyzing port infrastructure footprint..."):
                                nmap_prompt = f"Analyze this Nmap port scan data conservatively. Do not suggest or hallucinate vulnerabilities or CVEs unless explicitly stated:\n\n{nmap_out}"
                                nmap_analysis = llm.invoke(nmap_prompt)
                                st.markdown(nmap_analysis)
                                st.session_state.messages.append({"role": "assistant", "content": nmap_analysis})
                        else:
                            st.text(nmap_out)
                            st.session_state.messages.append({"role": "assistant", "content": nmap_out})
                        add_history(target_domain, "Nmap Port Scan", "LOW", nmap_out)
                    else:
                        st.error("Please provide a valid target domain for port mapping.")

                # ROUTE 3: DNS MONITOR MAP INTERFACE DIRECT LINK
                elif "dig" in prompt_lower or "dns" in prompt_lower:
                    dom_match = re.search(r"([a-zA-Z0-9.-]+\.[a-zA-Z]{2,})", prompt)
                    if dom_match:
                        target_domain = dom_match.group(1)
                        add_log(f"DNS Dig record check initiated for: {target_domain}")
                        with st.spinner("🌍 Performing DNS zone lookup..."):
                            dns_out = run_dig(target_domain)
                        st.markdown(f"```text\n{dns_out}\n```")
                        st.session_state.messages.append({"role": "assistant", "content": f"```text\n{dns_out}\n```"})
                        add_history(target_domain, "DNS Dig Audit", "LOW", dns_out)
                    else:
                        st.error("Please provide a valid domain target for DNS query mapping.")

                # ROUTE 4: WHOIS REGISTRAR MAP INTERFACE DIRECT LINK
                elif "whois" in prompt_lower:
                    dom_match = re.search(r"([a-zA-Z0-9.-]+\.[a-zA-Z]{2,})", prompt)
                    if dom_match:
                        target_domain = dom_match.group(1)
                        add_log(f"WHOIS lookup record collection initiated for: {target_domain}")
                        with st.spinner("🕵️ Querying registrar records..."):
                            whois_out = run_whois(target_domain)
                        st.markdown(f"```text\n{whois_out}\n```")
                        st.session_state.messages.append({"role": "assistant", "content": f"```text\n{whois_out}\n```"})
                        add_history(target_domain, "WHOIS Lookup", "LOW", whois_out)
                    else:
                        st.error("Please provide a valid domain target for WHOIS intelligence.")

                # GENERAL CONVERSATIONAL ANALYSIS COMMAND ROUTER FALLBACK
                else:
                    if llm:
                        with st.spinner("🤖 Processing query under strict SOC matrix scope..."):
